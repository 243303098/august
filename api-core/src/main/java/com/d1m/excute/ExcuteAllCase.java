...
public class ExcuteAllCase {
    ...
    public void excuteSingleAPI(CaseDetailsEntity caseDetailsEntity) {
        ...
        if (caseDetailsEntity.getInitSql().size() > 0) {
            ...
            for (int i = 0; i < caseDetailsEntity.getInitSql().size(); i++) {
                ...
                for (int j = 0; j < initSqlList.size(); j++) {
                    //  参数替换
                    caseDetailsEntity.getInitSql().set(i, caseDetailsEntity.getInitSql().get(i).replace("${" + initSqlList.get(i).toString() + "}", processorMap.get(caseDetailsEntity.getId()).get(initSqlList.get(i))));
                }
            }
        ...
        if (!StringTools.isNullOrEmpty(caseDetailsEntity.getRelatedApi())) {
            ...
            //  通过“.”来分隔关联的API
            String[] relatedAPI = caseDetailsEntity.getRelatedApi().split("\\&");
        ...
        for (int i = 0; i < head.length; i++) {
            ...
            for (int j = 0; j < headParamList.size(); j++) {
                ...
                head[i] = head[i].replace("${" + headParamList.get(j).toString() + "}", processorMap.get(caseDetailsEntity.getId()).get(headParamList.get(j)));
            }
        }
        ...
        if (!StringTools.isNullOrEmpty(caseDetailsEntity.getRequestParam())) {
            ...
            for (int i = 0; i < requestParamList.size(); i++) {
                //  参数替换
                caseDetailsEntity.setRequestParam(caseDetailsEntity.getRequestParam().replace("${" + requestParamList.get(i).toString() + "}", processorMap.get(caseDetailsEntity.getId()).get(requestParamList.get(i))));
            }
        }
        ...
        if (caseDetailsEntity.getRequestMethod().toUpperCase().trim().equals("POST")) {
            ...
            if (caseDetailsEntity.getHead().toUpperCase().contains("FORM")) {
                ...
                String[] formdata = caseDetailsEntity.getRequestMethod().split("\\&");
        ...
        Reporter.log("接口返回的Response为： " + result);
        ...
        if (caseDetailsEntity.getQuerySql().size() > 0 && !StringTools.isNullOrEmpty(caseDetailsEntity.getExpectSqlResult())) {
            ...
            for (int i = 0; i < caseDetailsEntity.getQuerySql().size(); i++) {
                ...
                for (int j = 0; j < querySqlList.size(); j++) {
                    //  参数替换
                    caseDetailsEntity.getQuerySql().set(i, caseDetailsEntity.getQuerySql().get(i).replace("${" + querySqlList.get(i).toString() + "}", processorMap.get(caseDetailsEntity.getId()).get(querySqlList.get(i))));
                }
            }
            //  获取sql返回值
        ...
        if (caseDetailsEntity.getClearSql().size() > 0) {
            ...
            for (int i = 0; i < caseDetailsEntity.getClearSql().size(); i++) {
                ...
                for (int j = 0; j < clearSqlList.size(); j++) {
                    //  参数替换
                    caseDetailsEntity.getClearSql().set(i, caseDetailsEntity.getClearSql().get(i).replace("${" + clearSqlList.get(i).toString() + "}", processorMap.get(caseDetailsEntity.getId()).get(clearSqlList.get(i))));
                }
            }
